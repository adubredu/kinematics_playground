...
for i=1:length(xb)
    atframe(anim, i) do 
        settransform!(mvis.visualizer, Translation(xb[i], 0, 0.9))
        ld = Point3D(root_frame(mechanism), xl[i], -0.15, -0.9) 
        jacobian_transpose_ik!(state, lbody, lfootpoint, ld)
        set_configuration!(mvis, configuration(state) )
        rd = Point3D(root_frame(mechanism), xr[i], 0.15, -0.9) 
